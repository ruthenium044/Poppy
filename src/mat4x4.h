...
struct mat4x4
{
    ...
    mat4x4(float value)
        : m00(value), m01(0.0f), m02(0.0f), m03(0.0f), m10(0.0f), m11(value), m12(0.0f), m13(0.0f), m20(0.0f),
          m21(0.0f), m22(value), m23(0.0f), m30(0.0f), m31(0.0f), m32(0.0f), m33(value)
    {
    }

    mat4x4(float m00, float m01, float m02, float m03, float m10, float m11, float m12, float m13, float m20, float m21,
...
{
    float4 result = {mat.m00 * vec.x + mat.m01 * vec.y + mat.m02 * vec.z + mat.m03 * vec.w,
...
mat4x4 mat4_perspective(float angles, float aspect, float near, float far)
{
    mat4x4 result = mat4x4(1.0f);
    float rad = toRad(angles);
    ...
    result.elements[14] = (2.0f * far * near) * nf;
    return result;
}
